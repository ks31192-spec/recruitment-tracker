...
router.post('/', (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO communication_log (candidate_id, application_id, comm_type, direction, summary, outcome, follow_up_date, logged_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
...
router.get('/follow-ups', (req, res) => {
  const rows = db.prepare(`SELECT cl.*, c.full_name, c.phone FROM communication_log cl
