...
router.get('/:id', (req, res) => {
  const v = db.prepare(`SELECT v.*, d.name as department_name, des.title as designation_title, ay.label as academic_year
  ...
  const stageCounts = db.prepare(`SELECT current_stage, COUNT(*) as count FROM applications WHERE vacancy_id = ? GROUP BY current_stage`).all(req.params.id);
...
router.post('/', authorize('super_admin', 'admin', 'hr'), (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO vacancies (title, department_id, designation_id, subject, qualification_required, experience_min, experience_max, salary_range_min, salary_range_max, positions_count, academic_year_id, description, created_by)
...
router.put('/:id', authorize('super_admin', 'admin', 'hr'), (req, res) => {
  ...
  db.prepare(`UPDATE vacancies SET title=?, department_id=?, designation_id=?, subject=?, qualification_required=?, experience_min=?, experience_max=?, salary_range_min=?, salary_range_max=?, positions_count=?, status=?, academic_year_id=?, description=?, updated_at=datetime('now') WHERE id=?`)
...
router.post('/:id/clone', authorize('super_admin', 'admin', 'hr'), (req, res) => {
  const orig = db.prepare('SELECT * FROM vacancies WHERE id = ?').get(req.params.id);
...
router.get('/:id/applications', (req, res) => {
  const apps = db.prepare(`SELECT a.*, c.full_name, c.phone, c.photo_path, c.source
