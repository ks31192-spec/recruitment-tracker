...
router.post('/', (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO interviews (application_id, interview_type, scheduled_date, scheduled_time, mode, location_or_link, demo_topic, demo_class, demo_duration_minutes)
  ...
  if (panel_member_ids?.length) {
    const ins = db.prepare('INSERT INTO interview_panel (interview_id, user_id) VALUES (?, ?)');
    panel_member_ids.forEach(uid => ins.run(r.lastInsertRowid, uid));
  }
...
router.put('/:id', (req, res) => {
  ...
  db.prepare(`UPDATE interviews SET scheduled_date=?, scheduled_time=?, mode=?, location_or_link=?, status=?, demo_topic=?, demo_class=?, demo_duration_minutes=?, updated_at=datetime('now') WHERE id=?`)
...
router.post('/:id/evaluate', (req, res) => {
  ...
  try {
    const r = db.prepare(`INSERT INTO evaluations (interview_id, evaluator_id, subject_knowledge, communication, teaching_aptitude, confidence_personality, tech_comfort, overall_impression, strengths, concerns, recommendation, private_notes)
...
router.get('/:id/evaluations', (req, res) => {
  let evals = db.prepare(`SELECT e.*, u.name as evaluator_name FROM evaluations e JOIN users u ON e.evaluator_id = u.id WHERE e.interview_id = ?`).all(req.params.id);
...
router.get('/schedule/today', (req, res) => {
  ...
  const rows = db.prepare(`SELECT i.*, a.candidate_id, c.full_name, c.phone, v.title as vacancy_title,
...
router.get('/schedule/upcoming', (req, res) => {
  const today = new Date().toISOString().slice(0, 10);
  const rows = db.prepare(`SELECT i.*, c.full_name, c.phone, v.title as vacancy_title
    FROM interviews i JOIN applications a ON i.application_id = a.id
    JOIN candidates c ON a.candidate_id = c.id JOIN vacancies v ON a.vacancy_id = v.id
