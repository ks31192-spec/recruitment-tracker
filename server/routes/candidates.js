...
router.get('/', (req, res) => {
  ...
  const countSql = sql.replace(/SELECT c\.\*.*FROM candidates c/, 'SELECT COUNT(*) as total FROM candidates c');
  const total = db.prepare(countSql).get(...params)?.total || 0;
  sql += ' ORDER BY c.created_at DESC LIMIT ? OFFSET ?';
  params.push(limit, offset);
  res.json({ success: true, data: { candidates: db.prepare(sql).all(...params), total, page: +page, pages: Math.ceil(total / limit) } });
});
...
router.get('/duplicates-check', (req, res) => {
  ...
  let dupes = [];
  if (phone) {
    dupes = db.prepare('SELECT id, full_name, phone, photo_path FROM candidates WHERE phone = ?').all(phone);
  }
  if (!dupes.length && name) {
    ...
    dupes = father_name ? db.prepare(sql).all(`%${name}%`, `%${father_name}%`) : db.prepare(sql).all(`%${name}%`);
  }
...
router.get('/:id', (req, res) => {
  ...
  c.qualifications = db.prepare('SELECT * FROM candidate_qualifications WHERE candidate_id = ?').all(c.id);
  c.experience = db.prepare('SELECT * FROM candidate_experience WHERE candidate_id = ?').all(c.id);
  c.applications = db.prepare(`SELECT a.*, v.title as vacancy_title, v.subject, d.name as department_name
  ...
  c.documents = db.prepare('SELECT * FROM documents WHERE candidate_id = ?').all(c.id);
  res.json({ success: true, data: c });
});

router.post('/', authorize('super_admin', 'admin', 'hr'), (req, res) => {
  ...
  if (!force && phone) {
    const dupes = db.prepare('SELECT id, full_name, phone FROM candidates WHERE phone = ?').all(phone);
    if (dupes.length) return res.json({ success: true, data: { duplicates: dupes } });
  }

  const r = db.prepare(`INSERT INTO candidates (full_name, father_or_husband_name, gender, date_of_birth, phone, whatsapp_number, email, current_city, current_state, source, referrer_name, notes, created_by)
...
router.put('/:id', authorize('super_admin', 'admin', 'hr'), (req, res) => {
  ...
  db.prepare(`UPDATE candidates SET full_name=?, father_or_husband_name=?, gender=?, date_of_birth=?, phone=?, whatsapp_number=?, email=?, current_city=?, current_state=?, source=?, referrer_name=?, notes=?, updated_at=datetime('now') WHERE id=?`)
...
router.get('/:id/timeline', (req, res) => {
  const events = [];
  const stages = db.prepare(`SELECT ash.*, a.vacancy_id, v.title as vacancy_title, u.name as changed_by_name
  ...
  const interviews = db.prepare(`SELECT i.*, v.title as vacancy_title
  ...
  const comms = db.prepare(`SELECT cl.*, u.name as logged_by_name FROM communication_log cl
...
router.post('/:id/qualifications', authenticate, (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO candidate_qualifications (candidate_id, degree, specialization, university, year_of_passing, percentage_or_cgpa, is_bed, is_deled, ctet_score, stet_score, net_qualified)
...
router.post('/:id/experience', authenticate, (req, res) => {
  ...
  const r = db.prepare(`INSERT INTO candidate_experience (candidate_id, school_name, designation, from_date, to_date, reason_for_leaving, reference_contact)
