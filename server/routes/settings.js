...
router.post('/departments', authorize('super_admin', 'admin'), (req, res) => {
  const { name } = req.body;
...
router.put('/departments/:id', authorize('super_admin', 'admin'), (req, res) => {
  const { name } = req.body;
  db.prepare('UPDATE departments SET name = ? WHERE id = ?').run(name, req.params.id);
  res.json({ success: true, data: { id: +req.params.id, name } });
});

router.delete('/departments/:id', authorize('super_admin', 'admin'), (req, res) => {
...
router.get('/designations', (req, res) => {
  const rows = db.prepare('SELECT * FROM designations ORDER BY title').all();
  res.json({ success: true, data: rows });
});

router.post('/designations', authorize('super_admin', 'admin'), (req, res) => {
  const { title } = req.body;
  if (!title) return res.status(400).json({ success: false, error: 'Title required' });
  try {
    const r = db.prepare('INSERT INTO designations (title) VALUES (?)').run(title);
...
router.put('/designations/:id', authorize('super_admin', 'admin'), (req, res) => {
  const { title } = req.body;
  db.prepare('UPDATE designations SET title = ? WHERE id = ?').run(title, req.params.id);
  res.json({ success: true, data: { id: +req.params.id, title } });
});

router.delete('/designations/:id', authorize('super_admin', 'admin'), (req, res) => {
...
router.get('/academic-years', (req, res) => {
  const rows = db.prepare('SELECT * FROM academic_years ORDER BY start_date DESC').all();
  res.json({ success: true, data: rows });
});

router.post('/academic-years', authorize('super_admin', 'admin'), (req, res) => {
  ...
  const r = db.prepare('INSERT INTO academic_years (label, start_date, end_date, is_current) VALUES (?, ?, ?, ?)').run(label, start_date, end_date, is_current ? 1 : 0);
  res.json({ success: true, data: { id: r.lastInsertRowid, label, start_date, end_date, is_current: !!is_current } });
});

router.put('/academic-years/:id', authorize('super_admin', 'admin'), (req, res) => {
  const { label, start_date, end_date, is_current } = req.body;
  if (is_current) db.prepare('UPDATE academic_years SET is_current = 0').run();
  db.prepare('UPDATE academic_years SET label = ?, start_date = ?, end_date = ?, is_current = ? WHERE id = ?').run(label, start_date, end_date, is_current ? 1 : 0, req.params.id);
  res.json({ success: true, data: { id: +req.params.id, label, start_date, end_date, is_current: !!is_current } });
});

router.delete('/academic-years/:id', authorize('super_admin', 'admin'), (req, res) => {
...
router.post('/users', authorize('super_admin'), (req, res) => {
  ...
  try {
    const r = db.prepare('INSERT INTO users (name, email, password_hash, role) VALUES (?, ?, ?, ?)').run(name, email, hash, role);
...
router.put('/users/:id', authorize('super_admin'), (req, res) => {
